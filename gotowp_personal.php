...
if ( is_admin() && ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) ) {
	...
	<div class="wrap">
		...
		<table id="gotowp_personal_sample_table" class="">
			<tr class="heading_row"><th colspan="2" class="heading"><?php _e('Sample Usage of Shortcode'); ?><br/></th></tr>
	...
	function webinar_get_registration_fields($web_key)
	{
		  $webinar_option_key='gotowp_personal_webinar_form_id_'.$web_key;
		   ...
		   if ( get_option( $webinar_option_key) !== false ) {
				...
				$response= curl_exec($curl);
				curl_close($curl);	
				add_option( $webinar_option_key, $response, '', 'yes' );
			}		
	...
	function gotowp_personal_get_webinar($webinarKey){
        ...
        if ( get_option( $webinar_option_key) !== false ) {
			...
			$curl_response = curl_exec($curl);
			curl_close($curl);
			add_option( $webinar_option_key, $curl_response, '', 'yes' );			
